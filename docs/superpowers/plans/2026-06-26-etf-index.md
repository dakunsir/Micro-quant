...
- Modify `zer0share/schema.py`: add `ETF_INDEX_COLS`.
- Modify `zer0share/catalog.py`: import `ETF_INDEX_COLS` and add `ETF_INDEX_SPEC`.
- Modify `zer0share/fetcher.py`: import `ETF_INDEX_COLS` and add `TushareFetcher.fetch_etf_index`.
- Modify `zer0share/sync/etf.py`: add `etf_index` snapshot job.
- Modify `zer0share/query/etf.py`: add local `etf_index` query function.
- Modify `zer0share/api.py`: add `LocalPro.etf_index`, dispatch mapping, and date validation for `pub_date` / `base_date`.
- Modify `zer0share/cli.py`: include `etf_index` in `ETF_TABLES`.
...
In `zer0share/catalog.py`, import `ETF_INDEX_COLS` and add under the ETF section:
...
In `zer0share/fetcher.py`, import `ETF_INDEX_COLS` and add after `fetch_etf_basic`:
...
In `zer0share/sync/etf.py`, import `ETF_INDEX_SPEC` and add a second job:
...
In `zer0share/query/etf.py`, import `ETF_INDEX_SPEC` and add:
...
In `zer0share/api.py`, update `_check_dates` to include `pub_date` and `base_date`:
