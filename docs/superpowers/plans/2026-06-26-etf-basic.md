...
- Modify `zer0share/schema.py`: add `ETF_BASIC_COLS`.
- Modify `zer0share/catalog.py`: add `ETF_BASIC_SPEC`.
- Modify `zer0share/sources/tushare.py`: import `ETF_BASIC_COLS` and add `fetch_etf_basic`.
- Create `zer0share/sync/etf.py`: register the `etf_basic` snapshot sync job.
- Modify `zer0share/pipeline.py`: include ETF jobs in the registry.
- Create `zer0share/query/etf.py`: implement local `etf_basic` filtering.
- Modify `zer0share/api.py`: expose `LocalPro.etf_basic` and `query("etf_basic")`.
- Modify `zer0share/cli.py`: add `ETF_TABLES`, `--etf`, and include `etf_basic` in choices/status.
- Modify `README.md`: document sync, query, and storage.
- Modify `skills/zer0share-data/references/api.md`: document local `etf_basic`.
...
- Modify: `zer0share/sources/tushare.py`
...
In `zer0share/schema.py`, after `INDEX_DAILY_COLS`, add:
...
In `zer0share/catalog.py`, import `ETF_BASIC_COLS` from `zer0share.schema`, then add this section after `INDEX_WEIGHT_SPEC`:
...
In `zer0share/sources/tushare.py`, import `ETF_BASIC_COLS` with the other schema constants. Add this method after `fetch_index_daily`:
...
```bash
git add zer0share/schema.py zer0share/catalog.py zer0share/sources/tushare.py tests/test_fetcher.py
...
In `zer0share/pipeline.py`, change the Tushare module import and loop to include `etf`:
...
```bash
git add zer0share/sync/etf.py zer0share/pipeline.py zer0share/cli.py tests/test_pipeline.py tests/test_cli.py
...
Expected: FAIL because `LocalPro.etf_basic` and `zer0share.query.etf` do not exist yet.
...
In `zer0share/api.py`, import `etf`:
...
In `skills/zer0share-data/references/api.md`, add:
