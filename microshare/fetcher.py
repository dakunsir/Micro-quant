...
class TushareFetcher:
    ...
    def __init__(self, token: str):
        ...
        http_url = os.environ.get("TUSHARE_HTTP_URL", self.DEFAULT_HTTP_URL).strip()
        if http_url:
    ...
    def fetch_basic(self) -> pd.DataFrame:
        ...
        if df is None or df.empty:
            logger.warning("stock_basic combined list_status returned no rows; retrying individual statuses")
            frames = []
            for status in ("L", "D", "P", "G"):
                part = self._pro.stock_basic(
                    exchange="",
                    list_status=status,
                    fields=fields,
                )
        ...
        while True:
            ...
            for attempt, delay in enumerate((*retry_delays, None), start=1):
                try:
                    ...
                    error_text = str(exc)
                    if frames and (
                        "查询数据失败，请确认参数" in error_text
                        or "offset" in error_text.lower()
                    ):
                        logger.warning(
...
def _select_columns_or_empty(df: pd.DataFrame | None, columns: list[str]) -> pd.DataFrame:
    ...
    if missing:
        logger.warning(f"Tushare response missing columns: {missing}")
