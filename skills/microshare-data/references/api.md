# Microshare Local Query API

This reference lists the local Tushare-like interfaces exposed by `microshare.pro_api()`.
...
`pro_api(config_path="config/settings.toml")` loads `data_dir` from the Microshare config.
...
Microshare does not currently expose local finance statements, macro data, realtime quotes, news, announcements, non-ETF fund interfaces, Hong Kong stocks, or US stocks. If users ask for these, state that the local skill cannot answer from current synced Microshare data.
