---
name: micro-data
description: 面向中文自然语言的 Micro 本地金融数据查询技能。用于把股票、ETF、指数、行业、期货、期权、股票池、复权行情等请求转成 Micro 的 Tushare-like 本地查询流程。适用于已经用 Micro 同步到本地 Parquet/DuckDB 数据的 A 股、ETF、期货和期权研究场景；查询不访问在线 Tushare，不消耗 Tushare 积分。
---

# Micro-data

Use this skill to answer Chinese financial data requests with local Micro data.
The query API intentionally looks like Tushare Pro, but the data source is local Parquet files queried through DuckDB.

## Core Rules

- Use `from micro import pro_api` as the default entry point.
...
- Never fabricate missing market, financial, news, macro, or real-time data. Micro only supports the local interfaces present in the current repository.

## Environment Check

Before running a query, verify the local project can import micro:

```bash
uv run python -c "from micro import pro_api; print(type(pro_api()).__name__)"
```
...
```bash
uv run python skills/micro-data/scripts/query_local.py daily \
...
- filters too narrow;
- requested interface is not implemented by Micro.
