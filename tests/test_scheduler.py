...
def test_start_scheduler_registers_basic_job_as_daily(tmp_path):
    ...
    assert cron_calls[0] == {"hour": 16, "minute": 0}  # trade_cal
    assert cron_calls[1] == {"hour": 18, "minute": 0}
    assert cron_calls[2] == {"hour": 18, "minute": 0}
    assert cron_calls[3] == {"hour": 8}
    assert cron_calls[4] == {"hour": 18, "minute": 5}
    assert cron_calls[5] == {"hour": 17, "minute": 0}
    assert cron_calls[6] == {"hour": 17, "minute": 10}
    assert cron_calls[7] == {"hour": 17, "minute": 20}
    assert cron_calls[8] == {"hour": 17, "minute": 30}
    assert cron_calls[9] == {"hour": 17, "minute": 40}
    assert cron_calls[10] == {"hour": 17, "minute": 50}
    assert cron_calls[11] == {"hour": 18, "minute": 0}
    assert cron_calls[12] == {"hour": 18, "minute": 10}
    assert cron_calls[13] == {"hour": 18, "minute": 20}
    assert cron_calls[14] == {"hour": 18, "minute": 30}
    assert cron_calls[15] == {"hour": 18, "minute": 40}
    assert cron_calls[16] == {"hour": 18, "minute": 50}   # opt_basic (offset 110)
    assert cron_calls[17] == {"hour": 19, "minute": 0}    # opt_daily (offset 120)
