...
from microshare.config import FeishuNotifierConfig, NotifierConfig, WeComNotifierConfig
from microshare.notifier import CompositeNotifier, FeishuNotifier, NullNotifier, WeComNotifier, build_notifier
...
def test_wecom_send_posts_text_payload():
    ...
    assert payload == {
        "msgtype": "text",
        "text": {"content": "[Microshare] 同步完成：成功 5 天"},
    }
...
def test_feishu_builds_interactive_card_like_Microfactor():
    n = FeishuNotifier("https://open.feishu.cn/open-apis/bot/v2/hook/fake", app="Microshare")

    card = n._build_card(
        title="[Microshare] `ricequant_history` 完成",
        color="green",
    ...
    assert card["card"]["header"]["title"]["content"] == "[Microshare] `ricequant_history` 完成"
...
def test_feishu_notify_stage_done_sends_green_card():
    ...
    assert card["card"]["header"]["template"] == "green"
    assert "[Microshare] `2026-05` 完成" in card["card"]["header"]["title"]["content"]
