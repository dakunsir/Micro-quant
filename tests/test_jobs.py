...
import pandas as pd
import pytest
from microshare.storage import MetaStore, DailyPartitionStore, SnapshotStore
from microshare.trading_calendar import TradingCalendar
from microshare.sync import SyncRuntime
from microshare.sync._jobs import DailySyncJob, SnapshotSyncJob, _format_duration
...
def test_daily_sync_job_writes_partition(tmp_path):
    ...
    with patch("microshare.sync._jobs.time") as mock_time:
        job.run(rt)
...
def test_daily_sync_job_logs_start_range_and_trading_day_count(tmp_path):
    ...
    with (
        patch("microshare.sync._jobs.time"),
        patch("microshare.sync._jobs.logger.info") as log_info,
    ):
        job.run(rt)
...
def test_daily_sync_job_progress_log_includes_elapsed_and_eta(tmp_path):
    ...
    with (
        patch("microshare.sync._jobs.PROGRESS_INTERVAL", 1),
        patch("microshare.sync._jobs.time.sleep"),
        patch("microshare.sync._jobs.time.monotonic", side_effect=[0, 10, 20, 20]),
        patch("microshare.sync._jobs.logger.info") as log_info,
    ):
        job.run(rt)
...
def test_daily_sync_job_starts_from_spec_first_date_when_never_synced(tmp_path):
    ...
    with patch("microshare.sync._jobs.time"):
        job.run(rt)
...
def test_daily_sync_job_raises_on_fetch_error(tmp_path):
    ...
    with patch("microshare.sync._jobs.time"), pytest.raises(RuntimeError):
        job.run(rt)
