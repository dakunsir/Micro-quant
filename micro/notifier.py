...
from micro.config import NotifierConfig
...
class WeComNotifier:
    ...
    def send(self, message: str) -> None:
        self._post_text(f"[Micro] {message}")
...
class FeishuNotifier:
    def __init__(self, webhook_url: str, *, app: str = "Micro") -> None:
        self._url = webhook_url
        self._app = app
...
def build_notifier(cfg: NotifierConfig) -> NotifierProtocol:
    ...
    if cfg.feishu.enabled and cfg.feishu.webhook_url:
        notifiers.append(FeishuNotifier(cfg.feishu.webhook_url, app="Micro"))
    if not notifiers:
        return NullNotifier()
    if len(notifiers) == 1:
        return notifiers[0]
