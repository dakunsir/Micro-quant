...
Both the query layer (micro/query/*.py) and the sync layer
(micro/sync/*.py) import specs from here so the two layers always
stay in sync with each other.  This module contains no logic — only
named constants.
"""

from micro.query.repository import DailyTableSpec, TableSpec
from micro.schema import (
    ADJ_FACTOR_COLS,
    BASIC_COLS,
    CI_MEMBER_COLS,
    DAILY_BASIC_COLS,
